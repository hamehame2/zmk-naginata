...
　ZMK Charlieplexingのキーボードにて同時打鍵判定において、google IMEや紅皿といったソフト側での対応では同時押しの成功率が低くなり実用に耐えなかったためです。（S, I, Oキーと他キーの同時押しにおいて）
 ...
 一方eswaiさんのzmk薙刀式やこちらのように新下駄に改変したものは動作OKでした。
 ...
 紅皿はノートPCでは電源に接続していな勝手理などでパフォーマンスが低いモード時には誤打が多くなる。
 　
...
キーボードで完結できるのは便利であるし、PCを共有しても問題がなく、かな入力の生存戦略としてソフトもいつま使用できるかという心配を減らせることも大きい。
...
nagifuncをいじってしまっているので、MacみLinuxの方は下記のほかに、nagifuncはeswai様のにしてもらう必要があります。
...
上記の薙刀ON/OFFのコンボ発動時LANGの切り替えがデフォなのですが、

 アプリによってはIMEが変わらず再度コンボを押す必要があるため、筆者は面倒に感じたため例えばng ONの場合は下記のように変更しています。
            //bindings = <&macro_tap &kp LANG1 &to NAGINATA>;
            bindings = <&macro_tap &kp CAPS &to NAGINATA>;
　
 AHK側でcapsにてIMEをOFFの半角英数直接入力とIME ONのひらがなを変更するようにしています（AHKを設定していないと上記コードは無意味です）。
 
  間違えるとIMEとレイヤーを合わせるために2回コンボを押す必要性もありますが、アプリごとに意図せず入力モードがずれるのに気づけないのに比べたら非常に便利です。
